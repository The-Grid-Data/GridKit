...
const DEFAULT_QUERY = `query getAvailUrls {
    profileInfos(where: { root: { slug: { _eq: "avail" } } }, limit: 1) {
      name
      urls {
        url
        urlType {
...
function ProfileSearch() {
  ...
  const where = useMemo(
    () => buildProfileWhere(profileFilters),
    [profileFilters],
  );
  ...
  const { data: rawFacetData } = useGridQuery<
    Record<string, { _count: number }>
  >(facetQuery?.query ?? "", profileFilters as Record<string, unknown>, {
    enabled: !!facetQuery,
  });
  ...
  return (
    <div>
      <p
        style={{ color: "#666", fontSize: 13, marginTop: 0, marginBottom: 16 }}
      >
        Uses <code>useGridFilterOptions()</code> to load filter dimensions,{" "}
        <code>buildProfileWhere()</code> to build Hasura where clauses.
      </p>

      {filtersLoading && (
        <p style={{ color: "#666" }}>Loading filter options...</p>
      )}
      {filtersError && (
        <pre style={{ color: "#d32f2f", fontSize: 13 }}>
          {filtersError instanceof Error
      ...
      {filters && (
        ...
        >
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && setSubmittedSearch(search)
                }
          ...
          <MultiSelect
            label="Sector"
            options={filters.profileSectors}
            selected={selectedSectors}
            onChange={setSelectedSectors}
            counts={facetCounts?.sectors}
          />
          <MultiSelect
            label="Status"
            options={filters.profileStatuses}
      ...
      <div>
        ...
        {searchFetching && !searchLoading && (
          <p style={{ color: "#666" }}>Refetching...</p>
        )}
        {!hasFilters && (
          <p style={{ color: "#999", fontSize: 13 }}>
            Select filters or search to find profiles.
          </p>
        )}
        {searchError && (
          <pre
            style={{ color: "#d32f2f", whiteSpace: "pre-wrap", fontSize: 13 }}
          >
        ...
        {profiles?.profileInfos && (
          <div>
            <p style={{ fontSize: 13, color: "#666", marginBottom: 8 }}>
              ...
              {page * PAGE_SIZE + profiles.profileInfos.length}
              {facetCounts != null ? ` of ${facetCounts.total}` : ""} (page{" "}
              {page + 1})
            </p>
            <table
              style={{
                ...
                {profiles.profileInfos.map((p: Record<string, unknown>) => (
                  ...
                  >
                    ...
                    <td style={{ padding: "6px 8px", color: "#666" }}>
                      {(p.profileType as Record<string, string>)?.name ?? "—"}
                    </td>
                    <td style={{ padding: "6px 8px", color: "#666" }}>
                      {(p.profileSector as Record<string, string>)?.name ?? "—"}
                    </td>
                    <td style={{ padding: "6px 8px", color: "#666" }}>
                      {(p.profileStatus as Record<string, string>)?.name ?? "—"}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 12,
                  cursor:
                    profiles.profileInfos.length < PAGE_SIZE
                      ? "default"
                      : "pointer",
                  borderRadius: 4,
                  border: "1px solid #ccc",
                  background: "#fff",
                  color:
                    profiles.profileInfos.length < PAGE_SIZE ? "#ccc" : "#333",
                }}
              >
                Next
              </button>
